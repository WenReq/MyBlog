---
title: 如何压缩前端项目中 JS 的体积
lang: 'zh-CN'
---

1. terser 或 uglify，及流行的使用 Rust 编写的 `swc` 压缩混淆化 JS。
2. `gzip` 或 `brotli` 压缩，在网关处（nginx）开启。
...
7. code spliting，路由懒加载，只加载当前路由的包，按需加载其余的 chunk，首页 JS 体积变小 (PS: 此条不减小总体积，但减小首页体积)
