...
上述模板中 `ref='addSwiper'`、`const addSwiper = ref(null)` 的写法为 `Vue3` 对 `ref` 的写法，一定要在 `setup` 函数中 `return` 给 `template`。这样就可以通过 `addSwiper` 拿到 `DialogAddSwiper` 组件内部的方法，比如 `addSwiper.value.open()`，通过获取弹窗组件内的 `open` 方法拿到内部的各个属性。
